...
public class PTPMessageSenderRunner implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		/*
		 * // Normal Anonymous inner class for MessageCreator(I) template.send("queue1",
		 * new MessageCreator() {
		 * 
		 * @Override public Message createMessage(Session session) throws JMSException {
		 * Message message=session.createTextMessage("from Sender"+new Date()); return
		 * message; } });
		 */

		/*
		 * // LAMDA based Anonymous inner class for MessageCreator(I)
		 * template.send("queue1",ses->{ Message
		 * message=ses.createTextMessage("from Sender::"+new Date()); return message;
		 * });
		 */
		// LAMDA based Anonymous inner class for MessageCreator(I)
